...
$(()=>{
    ...
    `;
        ...
        $(".peopleAuthen a").click(()=>{
			console.log(nowImei)
            _callWe('1001006105','/WeChart/ApplyCustomerService',{'fmobile':userData.fmobile,'IMEI': fTrueTelAsImei },res=>{
                if(res.msgBody && res.msgBody.sta == "ok"){
                     localStorage.setItem('peopleAuthen',true);
                    $.toast('申请已提交！');
                }else{
		...
		// 认证成功返回首页重新登录
		$('#reLogin a').click(()=> {
			let _stateName = $('.stateNmae').text();
			let _route = './../../';
			if (_stateName == '认证成功'){
				$.toast('认证成功请返回首页重新登录',function(){
					location.href = `${_route}index.html`;
				}); 
			} else {
				$.toast('认证未成功！','cancel');
			}
