    ...
    new setTobar({
        'mainBtn':[
            {'name':"完成装车",'url':'wccyrLoadSearchHistory.html',onClick:function(){
				...
				// 完成装车-获取车辆所在地的经纬度进行实时跟踪并将数据放入本地缓存中
				document.addEventListener('plusready', onPlusReady, false);
				var JWd = {};
				var onPlusReady = window.setInterval(function(){
					plus.geolocation.getCurrentPosition(function(p){
						JWd = {'经度':p.coords.longitude,'纬度':p.coords.latitude};
						window.localStorage.setItem('CarsJwd',JSON.stringify(JWd));
						console.log(JSON.stringify(JWd))
					}, function(e){
